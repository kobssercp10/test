...
export default function Component() {
  ...
  const scrollLeft = (key: string) => {
    sectionRefs.current[key]?.scrollBy({ left: -150, behavior: "smooth" });
  };
    ...
    <div>
      ...
      {sections.map((section) => (
        <div key={section.key} className="mt-6 px-4">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex gap-2">
              <Button
                variant="ghost"
                size="icon"
                onClick={() => scrollLeft(section.key)}
              ...
              <Button
                variant="ghost"
                size="icon"
                onClick={() => scrollRight(section.key)}
                className="w-8 h-8 bg-pink-500 hover:bg-pink-600 rounded-full"
              >
          ...
          >
            {section.games.map((slug) => {
              const game = gameMap[slug];
              const [provider, name, gameId] = game.apiPath.split("/");
              return (
                <Link key={slug} href={`/game/${provider}/${name}/${gameId}`}>
